...
class APIExtensionKtTest {
    ...
    @Test
    fun changePropertyValueIgnoreItsType() {
        val demo = TestDemo()
        val originValue = demo.getPropertyValue("name")
        demo.changePropertyValueIgnoreItsType("name", TestDemo())
        val nowValue = demo.getPropertyValue("name")
        originValue.should.not.equal(nowValue)
        (nowValue is TestDemo).should.be.`true`
    }
